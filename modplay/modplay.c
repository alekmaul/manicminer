...
void MODFILE_term(MODFILE* mod_buf) {
  MODFILE_Stop(mod_buf);
  MODFILE_Free(mod_buf);
}
  
int MODFILE_loadmod(char *inFile, MODFILE* mod_buf,int freq, int channels, int bits, BOOL mixsigned) {
  ...
  if (retmod>=0) {
     ...
     *             of signed values
    */ 
    MODFILE_SetFormat(mod_buf, freq, channels, bits, mixsigned);
...
int MODFILE_loadmod_from_buffer(unsigned int len, unsigned char *pMod, MODFILE* mod_buf,int freq, int channels, int bits, BOOL mixsigned) {
...
int MODFILE_loadxm_from_buffer(unsigned int len, unsigned char *pMod, MODFILE* mod_buf,int freq, int channels, int bits, BOOL mixsigned) {
